  ...
  <view class="waterfalls-box" :style="{ height: height+'px' }">
    ...
    >
		<image
		  ...
		  :class="{ single }"
		  :style="imageStyle"
		  :src="item.works.fileType ===  'image' ?  item.works.url :  item.works.url+ '?x-oss-process=video/snapshot,t_1000,m_fast'"
		  @load="imageLoadHandle(index)"
		  @error="imageLoadHandle(index)"
		  @click="$emit('image-click', item._id, item.user_id[0]._id)"
		/>
      <slot name="slot{{item._id}}" />
    </view>
    <!--  #endif -->
