<template>
	<view class="login">
		<view class="content">
			...
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="userData" type="text" maxlength="11" placeholder="用户名" :focus="isFocus"
					@blur='isExist'></wInput>
...
<script>
	...
	export default {
		...
		methods: {
			async isExist() {
				if (this.userData.length > 11 || this.userData.length < 4 || /[^a-zA-Z0-9]/.test(this.userData) || this.isCheck) return
				this.isCheck = true
				uni.showLoading({
					mask: true,
					title: '检查中...'
				...
				try {
					const {
						result
					} = await database.collection('uni-id-users').where({
						username: this.userData
					}).field('avatar').get({
						getOne: true
					})
					if (result.data) {
						this.avatar = result.data.avatar || '/static/avatar.png'
						this.exist = true
					} else {
						uni.showModal({
							showCancel: false,
			...
			startLogin(e) {
				...
				if (this.userData.length > 11 || this.userData.length < 4 || /[^a-zA-Z0-9]/.test(this.userData)) {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '用户名错误'
